import chordConvention from './englishGerman.js';
import parser from './parser.js';
import builder from './builder.js';
import transponation from './transponation.js';
...
function conventionalizeOutput() {
    ...
    $chordElems.each(function () {
        ...
        var conventionalizedChord = chordConvention.getChordNameInConvention(normalizedName, convention);
...
function doTransponation() {
    ...
    var chordMapping;
    try {
        chordMapping = transponation.getChordMapping(chords, amount, dir);
    } catch ( error ) {
...
function render() {
    ...
    var $doc = $("#rendered");
    var parsedDoc = parser.parse(text);
    
    prepareParseTree(parsedDoc);

    builder.build($doc, parsedDoc);
...
function saveNormalizedFormOfChords() {
    ...
    $('.chord').each(function () {
        ...
        try {
            normalizedName = chordConvention.normalizeChord(normalizedOriginalName, convention);
        } catch ( error ) {
            setEditError(error);
            return;
        }
